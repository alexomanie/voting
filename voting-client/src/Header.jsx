...
export const Header = ({ membersOnline }) => {
  const { questions } = useStore((state) => ({
    questions: state.questions,
  }))
  return (
    <header className="text-white min-w-full container">
      <div className="bg-gray-600 min-w-full flex flex-wrap p-5 flex-col md:flex-row items-center justify-between">
        ...
        <div className="flex flex-row items-center self-center text-md items-center">
          <CSVLink
          className="text-white hover:text-gray-200 font-bold cursor-pointer mr-8"
            headers={[
              { label: "Frage", key: "question" },
              { label: "Stimmen", key: "votes" },
              { label: "Beantwortet: ", key: "complete" },
            ]}
            separator={";"}
            filename="Fragen.csv"
            data={questions.map((question) => ({
              ...
              complete: question.complete ? "Ja" : "Nein",
            }))}
          >
            Fragen exportieren
          </CSVLink>
          <div className="font-medium title-font">online: {membersOnline}</div>
